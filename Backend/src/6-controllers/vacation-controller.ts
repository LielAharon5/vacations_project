...
// GET http://localhost:3001/api/vacations/reports ------------------ Reports
router.get("/vacations/reports",[verifyLoggedIn, verifyAdmin], async(request:Request,response:Response,next:NextFunction)=>{
    try {
        const reports = await vacationLogic.getReports()
